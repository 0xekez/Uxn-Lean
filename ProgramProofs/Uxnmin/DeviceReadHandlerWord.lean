import ProgramProofs.Uxnmin.DeviceReaderWord
...
/-- The native word-mode DEI handler pops its port and pushes the corresponding device byte. -/
theorem handler_device_read_word (ram : Word → Byte) (softwareStack : Word) (pointer keep : Byte)
    ...
    (mode : ram 0x44#16 = 1#8) (kept : ram 0x2bf#16 = keep) (keepBound : keep.toNat ≤ 1)
    (cursor : ram (softwareStack + (0x100#16 + keep.setWidth 16)) = pointer)
    (ptr : ram (softwareStack + 0x100#16) = pointer)
    (workingSpace : working.ptr.toNat ≤ 246) (returnSpace : returning.ptr.toNat ≤ 247) :
    ...
    ∃ final, Reaches (machine ram 0x4b1 working (Stack.pushWord returning returnAddress)) final ∧
      ...
      final.mem.ram = Function.update (Function.update
        ...
        (softwareStack + 0x100#16) ((if keep = 0 then pointer - 1#8 else pointer) + 2#8))
        (softwareStack + ((if keep = 0 then pointer - 1#8 else pointer) + 1#8).setWidth 16)
        (ram (0x759 + (port + 1#8).setWidth 16)) ∧
  ...
  obtain ⟨final, readRun, finalPC, finalWP, finalRP, finalRAM, finalWF, finalRF⟩ :=
    device_reader_word popped.mem.ram softwareStack (if keep = 0 then pointer - 1#8 else pointer)
  ...
  · rw [finalRAM, outside _ (.inl (by decide)), outside _ (.inl (by decide)),
      outside _ (.inr (by bv_omega)), outside _ (.inr (by bv_omega)), popRAM]
    rfl
  · intro index below
