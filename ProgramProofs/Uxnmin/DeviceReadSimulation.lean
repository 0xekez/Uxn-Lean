import ProgramProofs.Uxnmin.DeviceReadHandler
...
/-- DEI, DEIk, DEIr, and DEIkr return to the next represented guest boundary. -/
theorem device_read_byte_simulation {guest outer : Uxn.State} (host : Uxn.Host.State)
  ...
  obtain ⟨pushed, handlerRun, handlerPC, handlerWorking, handlerReturning, handlerMemory, handlerFrame, handlerReturnFrame⟩ :=
    handler_device_read_byte dispatched.mem.ram (stackBase ((guest.mem.ram guest.pc).getLsbD 6))
      ...
      (by simpa only [byteMode, Bool.false_eq_true, if_false, BitVec.ofNat_eq_ofNat] using short) keep
  ...
  let guest' := deviceReadNext guest host ((guest.mem.ram guest.pc).getLsbD 6) false
  ...
  have represented : Represents guest' final := by
    apply ((rep.popStack ((guest.mem.ram guest.pc).getLsbD 6)
      ...
      ((host.read ((guestStack guest ((guest.mem.ram guest.pc).getLsbD 6)).data
        ((guestStack guest ((guest.mem.ram guest.pc).getLsbD 6)).ptr - 1))).setWidth 16)).transport
    change pushed.mem.ram = _
    rw [handlerMemory, portValue, readValue]
    cases hr : (guest.mem.ram guest.pc).getLsbD 6 <;> cases hk : (guest.mem.ram guest.pc).getLsbD 7 <;>
      simp [popStack, guestStack, pushStack]
  ...
  · simpa only [byteMode] using guest_device_read guest host opcode
