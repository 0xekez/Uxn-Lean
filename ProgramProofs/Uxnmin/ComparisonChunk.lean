...
/-- The comparison instruction family's complete chunk contract in the actual IO transition
system. Host records may differ; both executions preserve their own record. -/
theorem comparison_chunk (operator : ComparisonOp) {guest outer : Uxn.State}
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    comparison_simulation operator boundary confined opcode
