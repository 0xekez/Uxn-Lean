...
/-- The load instruction family's complete chunk contract in the actual IO transition
system. Host records may differ; both executions preserve their own record. -/
theorem load_chunk (kind : AddressMode) {guest outer : Uxn.State}
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    load_simulation kind boundary confined opcode firstBound secondBound
