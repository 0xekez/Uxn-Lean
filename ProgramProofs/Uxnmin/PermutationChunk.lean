...
/-- All five stack permutations have a complete chunk contract in the actual IO transition
system. Host records may differ; both executions preserve their own record. -/
theorem stack_chunk (operator : StackOp) {guest outer : Uxn.State}
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    stack_simulation operator boundary confined opcode
