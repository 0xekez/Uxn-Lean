...
/-- The arithmetic instruction family's complete chunk contract in the actual IO transition
system. Host records may differ; both executions preserve their own record. -/
theorem arithmetic_chunk (operator : ArithmeticOp) {guest outer : Uxn.State}
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    arithmetic_simulation operator boundary confined opcode
