...
import ProgramProofs.Uxnmin.PopTriple
...
theorem handler_rot (memory : Word → Byte) (softwareStack : Word) (pointer keep : Byte)
    ...
    ∃ final, Reaches
      (machine memory 0x3d7 working (Stack.pushWord returning returnAddress)) final ∧
      ...
      final.mem.ram = storeStackOperand
        (storeStackOperand
          (storeStackOperand
            (Function.update memory (softwareStack + (0x100#16 + keep.setWidth 16))
              (pointer - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8))) softwareStack
            (if keep = 0 then pointer - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) else pointer)
        ...
        (stackOperand memory softwareStack (pointer - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8)) short) short ∧
  ...
  obtain ⟨middle, before, middlePC, middleMem, middleCode, nativeWP,
    ...
    pop_triple memory softwareStack pointer keep short working returning returnAddress
  ...
  have middlePointer : middle.mem.ram (softwareStack + 0x100#16) =
      if keep = 0 then pointer - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) else pointer := by
    rw [middleMem]
    by_cases zero : keep = 0
    · simp [zero]
    · rw [Function.update_of_ne (by bv_omega), ptr, if_neg zero]
  have code3e0 : middle.mem.ram 0x3e0#16 = 0x25#8 := middleCode _ (by decide) (by decide) (by simp [MutableCode])
  have code3e1 : middle.mem.ram 0x3e1#16 = 0x25#8 := middleCode _ (by decide) (by decide) (by simp [MutableCode])
  obtain ⟨final, finalSteps, finalPC, finalWP, finalRP, finalMem, finalCode, finalWF, finalRF⟩ :=
    push_triple .rot middle.mem.ram softwareStack
      (if keep = 0 then pointer - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) - (if short then 2#8 else 1#8) else pointer)
  ...
  have shape : middle = machine middle.mem.ram 0x3e0 middle.mem.wstk middle.mem.rstk := by
    cases middle
    simpa [machine] using middlePC
  have after : Reaches middle final := by
    rw [shape, ← workingEta, ← returningEta]
    refine .next (native_rot _ _ _ _ _ _ _ code3e0) ?_
    exact .next (native_rot _ _ _ _ _ _ _ code3e1) finalSteps
