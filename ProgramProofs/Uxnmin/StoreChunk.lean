...
/-- The store instruction family's complete chunk contract in the actual IO transition
system. Host records may differ; both executions preserve their own record. -/
theorem store_chunk (kind : AddressMode) {guest outer : Uxn.State}
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    store_simulation kind boundary confined opcode firstBound secondBound
