...
def BytePopPairCode (entry : Word) : Prop :=
  ∀ (memory : Word → Byte) (working returning : Uxn.Stack) (offset : Word),
    ...
    Uxn.step (machine memory (entry + offset) working returning) = .done (.next
      (machine memory (if offset = 0 then 0x2b3 else 0x2ad) working
        (Stack.pushWord returning (entry + offset + 3))))

theorem pop_byte_pair (entry : Word) (calls : BytePopPairCode entry) (memory : Word → Byte)
    ...
    ∃ final, Reaches (machine memory entry working (Stack.pushWord returning returnAddress)) final ∧
      ...
      Stack.pushWord (Stack.pushWord { final.mem.wstk with ptr := working.ptr }
        (stackOperand memory softwareStack pointer false))
        (stackOperand memory softwareStack (pointer - 1#8) short) = final.mem.wstk ∧
  ...
  simpa [OperandKind.short] using
    pop_operand_pair .byte .mode entry
