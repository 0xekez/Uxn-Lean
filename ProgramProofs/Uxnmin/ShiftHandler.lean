import ProgramProofs.Uxnmin.BytePair
...
def shiftResult (shift value : Word) : Word :=
  value >>> ((shift.setWidth 8) &&& 0x0f#8).toNat <<< ((shift.setWidth 8) >>> 4).toNat

theorem shift_tail (memory : Word → Byte) (shift value : Word)
    (working returning : Uxn.Stack) (code : CodeImage memory)
    (workingSpace : working.ptr.toNat ≤ 251) :
    ∃ final, Reaches
      (machine memory 0x50f (Stack.pushWord (Stack.pushWord working shift) value) returning) final ∧
      final.pc = 0x28c ∧ final.mem.ram = memory ∧
      final.mem.wstk.ptr = working.ptr + 2#8 ∧
      final.mem.wstk.data working.ptr = ((shiftResult shift value) >>> 8).setWidth 8 ∧
      final.mem.wstk.data (working.ptr + 1#8) = (shiftResult shift value).setWidth 8 ∧
      final.mem.rstk = returning ∧
      (∀ address : Byte, address.toNat < working.ptr.toNat →
        final.mem.wstk.data address = working.data address) := by
  have code50f : memory 0x50f#16 = 0x24#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code510 : memory 0x510#16 = 0x03#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code511 : memory 0x511#16 = 0x3f#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code512 : memory 0x512#16 = 0x40#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code513 : memory 0x513#16 = 0xfd#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code514 : memory 0x514#16 = 0x77#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  iterate 4
    apply Reaches.prepend
    · simp [uxn_state, uxn_step, code50f, code510, code511, code512, code513, code514]
      rfl
...
theorem shift_calls : BytePopPairCode 0x509 := by
  intro memory working returning offset code position
  have code509 : memory 0x509#16 = 0x60#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code50a : memory 0x50a#16 = 0xfd#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code50b : memory 0x50b#16 = 0xa7#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code50c : memory 0x50c#16 = 0x60#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code50d : memory 0x50d#16 = 0xfd#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  have code50e : memory 0x50e#16 = 0x9e#8 := code _ (by decide) (by decide) (by simp [MutableCode])
  rcases position with rfl | rfl <;> apply jsi_call <;>
    simp [code509, code50a, code50b, code50c, code50d, code50e]

theorem handler_sft (memory : Word → Byte)
    ...
    ∃ final, Reaches
      (machine memory 0x509 working (Stack.pushWord returning returnAddress)) final ∧
      ...
      final.mem.ram = storeStackOperand
        (Function.update memory (softwareStack + (0x100#16 + keep.setWidth 16))
          (pointer - 1#8 - (if short then 2#8 else 1#8))) softwareStack
        (if keep = 0 then pointer - 1#8 - (if short then 2#8 else 1#8) else pointer)
        (shiftResult (stackOperand memory softwareStack pointer false)
          (stackOperand memory softwareStack (pointer - 1#8) short)) short ∧
  ...
  obtain ⟨second, before, secondPC, combinedMem, combinedCode, workingPointer,
    ...
    pop_byte_pair 0x509 shift_calls memory softwareStack pointer keep short
      working returning returnAddress selected code high low mode kept keepBound cursor workingSpace returnSpace
  have secondShape : second = machine second.mem.ram (0x509 + 6) second.mem.wstk second.mem.rstk := by
    cases second
  ...
  obtain ⟨computed, computedSteps, computedPC, computedMem, computedWP, computedHigh, computedLow, computedRP, computedWF⟩ :=
    shift_tail second.mem.ram (stackOperand memory softwareStack pointer false)
      (stackOperand memory softwareStack (pointer - 1#8) short)
  ...
  have computation : Reaches second computed := by
    rw [secondShape]
    simpa [workingEta] using computedSteps
  ...
  have computedPointer : computed.mem.ram (softwareStack + 0x100#16) =
      if keep = 0 then pointer - 1#8 - (if short then 2#8 else 1#8) else pointer := by
  ...
  have computedWorkingEta : Stack.pushWord { computed.mem.wstk with ptr := working.ptr }
      (shiftResult (stackOperand memory softwareStack pointer false)
        (stackOperand memory softwareStack (pointer - 1#8) short)) = computed.mem.wstk := by
