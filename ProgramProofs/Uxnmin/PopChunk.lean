...
/-- The POP instruction's complete chunk contract in the actual IO transition
system. Host records may differ; both executions preserve their own record. -/
theorem pop_chunk {guest outer : Uxn.State}
    ...
    (opcode : guest.mem.ram guest.pc &&& 0x1f = 2) :
    ∃ guest' first final,
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    pop_simulation boundary confined opcode
