...
import ProgramProofs.Uxnmin.ComparisonHandlers
import ProgramProofs.Uxnmin.GuestComparison
...
/-- The seven comparison operations in all eight modes, from one evaluation
boundary to the next, preserving device bookkeeping and the active native frame. -/
theorem comparison_simulation (operator : ComparisonOp) {guest outer : Uxn.State}
  ...
  have dispatchPC : dispatched.pc = operator.entry := by
    ...
    rw [boundary.code _ (by cases operator <;> decide) (by cases operator <;> decide)
      (by cases operator <;> simp [ComparisonOp.guestOpcode, MutableCode]),
      boundary.code _ (by cases operator <;> decide) (by cases operator <;> decide)
      (by cases operator <;> simp [ComparisonOp.guestOpcode, MutableCode])]
    cases operator <;> rfl
  obtain ⟨computed, comparisonRun, comparisonPC, comparisonWorking, comparisonReturning,
    comparisonMemory, comparisonCode, comparisonFrame, comparisonReturnFrame⟩ :=
    handler_comparison operator dispatched.mem.ram (stackBase ret)
  ...
  have run : Reaches outer computed := by
    ...
    simpa only [returnShape, ← dispatchPC, machine] using comparisonRun
  have nonzero : guest.mem.ram guest.pc ≠ 0#8 := by
    intro zero
    cases operator <;> simp [zero, ComparisonOp.guestOpcode] at opcode
  have computedTop : computed.mem.wstk.data 0#8 = guest.mem.ram guest.pc := by
    rw [comparisonFrame _ (by simp [working])]
    exact top
  let final : Uxn.State := { computed with pc := 0x16d, mem.wstk.ptr := 0 }
  have loop : Uxn.step computed = .done (.next final) :=
    return_to_loop computed comparisonCode comparisonPC (comparisonWorking.trans working)
      (computedTop ▸ nonzero)
  let guest' := comparisonNext guest operator ret short keep
  have represented : Represents guest' final := by
    apply ((rep.popStack ret keep (operandSize short + operandSize short)).storeOperand ret false
      ((operator.result (operand (guestStack guest ret) (guestStack guest ret).ptr short)
        (operand (guestStack guest ret) ((guestStack guest ret).ptr - operandSize short) short)).setWidth 16)).transport
    change computed.mem.ram = _
    rw [comparisonMemory, rep.operand, rep.operand]
    cases keep <;>
      simp [guestStack_pc, popStack_pointer, operandSize,
        BitVec.sub_eq_add_neg, BitVec.neg_add, BitVec.add_assoc]
  refine ⟨guest', final, guest_comparison operator guest opcode,
    run.trans (.next loop (.refl _)), ⟨represented, rfl, rfl, ?_⟩, ?_, ?_, comparisonReturning, ?_⟩
  · change computed.mem.rstk.ptr.toNat ≤ 245
    rw [comparisonReturning]
  ...
  · apply boundary.toRepresents.ne_of_pc_next represented
    have preserved (state : Uxn.State) (ret short keep : Bool) :
        (comparisonNext state operator ret short keep).pc = state.pc + 1 := by
  ...
  · intro address outside
    ...
    rw [comparisonMemory, storeStackOperand_frame _ ret false _ _ outside]
  ...
  · intro index below
    ...
    rw [comparisonReturnFrame _ below]
