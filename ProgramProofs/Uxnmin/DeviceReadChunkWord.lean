...
/-- Word-mode DEI is a silent native chunk even though its direct instruction
uses a host read request. -/
theorem device_read_word_chunk {guest outer : Uxn.State}
  ...
  obtain ⟨guest', final, guestStep, steps, related, different, memory, pointer, frame⟩ :=
    device_read_word_simulation guestHost boundary devices confined opcode wordMode
  cases steps with
